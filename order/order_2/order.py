...
def add_order(book_id):
  ...
  if temp['status']=="available":
    r2=requests.put("http://192.168.1.202:3000/bazar/decrease_quantity/"+str(book_id),{'amount':amount})
    temp2=r2.json()
    print(r2.json())
    new_order = Order(book_id)
    db.session.add(new_order)
    db.session.commit()
    r=requests.post("http://192.168.1.121:2000/bazar/purchase_con/"+str(book_id),data={'amount':amount})
