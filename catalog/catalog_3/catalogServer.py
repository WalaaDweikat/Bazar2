...
def decrease_book_quantity(book_id):
    ...
    if book:
        ...
        if book.quantity - amount < 0:
            ...
            book.quantity = 0  
            db.session.commit()
            r=requests.put("http://192.168.1.202:2000/bazar/decrease_quantity_con/"+str(book_id),data={'amount':amount})
            r=requests.put("http://192.168.1.202:3000/bazar/decrease_quantity_con/"+str(book_id),data={'amount':amount})
            r=requests.delete("http://192.168.1.121:5000/bazar/delete/"+str(book_id))
...
@app.route("/bazar/decrease_quantity_con/<book_id>",methods=['PUT'])
def decrease_book_quantity_con(book_id):
    book = Catalog.query.get(book_id)
    if book:
        #amount = request.json['amount']
        #args = request.args
...
def increase_book_quantity(book_id):
    ...
    if book:
        ...
        book.quantity = book.quantity + amount 
        db.session.commit()
        r=requests.put("http://192.168.1.202:2000/bazar/increase_quantity_con/"+str(book_id),data={'amount':amount})
        r=requests.put("http://192.168.1.202:3000/bazar/increase_quantity_con/"+str(book_id),data={'amount':amount})
        r=requests.delete("http://192.168.1.121:5000/bazar/delete/"+str(book_id))
        return jsonify({"msg" : f"increase of '{book.title}' book quantity,the quantity now is {book.quantity}"})
    else:  return jsonify({book_id : "this id does not exist"})


@app.route("/bazar/increase_quantity_con/<book_id>",methods=['PUT'])
def increase_book_quantity_con(book_id):
    book = Catalog.query.get(book_id)
    if book:
        amount = int(request.form.get('amount'))
        book.quantity = book.quantity + amount 
...
def update_book_price(book_id):
    ...
    if book:
        ...
        book.price = price 
        db.session.commit()
        r=requests.put("http://192.168.1.202:2000/bazar/update_price_con/"+str(book_id),data={'price':price})
        r=requests.put("http://192.168.1.202:3000/bazar/update_price_con/"+str(book_id),data={'price':price})
        r=requests.delete("http://192.168.1.121:5000/bazar/delete/"+str(book_id))
        return book_schema.jsonify(book)
    else:  return jsonify({book_id : "this id does not exist"})


@app.route("/bazar/update_price_con/<book_id>",methods=['PUT'])
def update_book_price_con(book_id):
    book = Catalog.query.get(book_id)
    if book:
        #price = request.json['price']
        #args = request.args
